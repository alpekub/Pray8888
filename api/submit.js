...
export default async function handler(req, res) {
  ...
  try {
    ...
    const tgName = `${telegram_first_name || ''} ${telegram_last_name || ''}`.trim() || '—';
...
📌 Telegram name: ${tgName}`;
    ...
    await telegram('sendMessage', {
      ...
      reply_markup: {
        inline_keyboard: [
          [
            { text: '✅ Одобрить', callback_data: `approve:${telegram_id}` },
            { text: '❌ Отклонить', callback_data: `decline:${telegram_id}` }
          ]
        ]
      }
    });
